...
class FlapjacksAndSasquatches extends Table
{
    ...
    */
    ...
    public function actPlayCard(int $card_id)
    {
        ...
        if (
            ...
            throw new BgaUserException(
                self::_("This card is not in your hand"),
            );
        }
    ...
    function actSelectTarget(int $target_id)
    {
        ...
        if ($targeted === "others" && $target_id == $player_id) {
            throw new BgaUserException(
                self::_("You cannot target yourself with this card"),
            );
        }
    ...
    function actSelectTreesToSwitch(
        ...
        if (!$my_tree || !$target_tree) {
            throw new BgaUserException(self::_("Invalid tree selection"));
        }
    ...
    function actPlayReaction(int $card_id)
    {
        ...
        if ($card_def["type"] !== "reaction") {
            throw new BgaUserException(
                self::_("This card cannot be played as a reaction"),
            );
        }
    ...
    */
    ...
    function resolvePaulBunyan($card_id)
    {
        ...
        foreach ($active_trees as $tree) {
            ...
            $new_score = self::getUniqueValueFromDB(
                "SELECT SUM(points_value) FROM cut_tree
                 WHERE player_id = " . $tree["player_id"],
            );
            self::DbQuery(
                "UPDATE player SET player_score = $new_score
                 WHERE player_id = " . $tree["player_id"],
            );
    ...
    function resolveSwitchTags($card_id, $target_id, $player_id)
    {
        ...
        self::DbQuery(
            "UPDATE player SET player_score = $my_new_score
             WHERE player_id = $player_id",
        );
        self::DbQuery(
            "UPDATE player SET player_score = $their_new_score
             WHERE player_id = $target_id",
        );
     ...
     * Active player may take target's active tree and discard their own.
     */
    function resolveSasquatchMating($card_id, $target_id, $player_id)
    {
        ...
        // Swap trees: take target's tree, discard own
        $my_tree = self::getObjectFromDB(
        ...
        self::notifyAllPlayers(
            "sasquatchMating",
            clienttranslate(
                '${player_name} loses their turn! ${active_name} takes their tree!',
            ),
            [
                "player_id" => $target_id,
                "player_name" => self::getPlayerNameById($target_id),
    ...
    function stPreChoppingRoll()
    {
        ...
        if (!$has_tree || !$this->playerHasChoppingTool($player_id)) {
            $player_name = self::getPlayerNameById($player_id);
            $reason = !$has_tree
                ? clienttranslate(
                ...
                : clienttranslate(
                    '${player_name} has no axe or Long Saw — chopping roll skipped',
                );
    ...
    function argChoppingRoll()
    {
        ...
        return [
            ...
            "has_apprentice" => $info["has_apprentice"],
    ...
    function getChoppingRollInfo($player_id)
    {
        ...
        return [
            ...
            "has_apprentice" => $apprentice !== null,
            "tool_name" => $tool_name,
            "long_saw" => $long_saw,
            "axe" => $axe,
        ];
    ...
    function actChopRoll()
    {
        ...
        if ($using_long_saw) {
            ...
            if (
                $breaks >= $this->game_constants["axe_break_threshold"] &&
                !$is_unbreakable
            ) {
                $axe_broke = true;
            }
        ...
        $apprentice_chop = 0;
        $apprentice = self::getObjectFromDB(
            "SELECT help_id, card_id FROM player_help
             WHERE player_id = $player_id AND help_type = 'apprentice'",
        );
        if ($apprentice) {
        ...
        // Clean up non-persistent modifiers
