...
export class Game {
    ...
    setup(gamedatas) {
        ...
        var html =
            '<div id="flapjacksandsasquatches_game_area">' +
                ...
                '</div>' +
                '<div id="player_areas">' + playerAreasHtml + '</div>' +
    ...
    setupDecks() {
        ...
        if (jackDeck) {
            jackDeck.style.backgroundImage = "url(" + g_gamethemeurl + "img/" + redBack.sprite + ")";
            jackDeck.style.backgroundPosition = redBgPosXPct + "% " + redBgPosYPct + "%";
        ...
        if (treeDeck) {
            treeDeck.style.backgroundImage = "url(" + g_gamethemeurl + "img/" + treeBack.sprite + ")";
            treeDeck.style.backgroundPosition = treeBgPosXPct + "% " + treeBgPosYPct + "%";
    ...
    displayTree(player_id, tree) {
        ...
        var cardHtml =
            ...
            "background-image: url(" + g_gamethemeurl + "img/tree_cards.jpg); " +
            "background-position: " + bgPosXPct + "% " + bgPosYPct + "%; " +
            "background-size: " + bgSizeWidth + "px auto;" +
            '"></div>';

        var html =
            '<div class="tree_display" id="tree_' + player_id + '" data-tree-id="' + tree.id + '">' +
            cardHtml +
            '<div class="tree_def_overlay">' +
            '<div class="tree_name">' + treeDef.name + "</div>" +
            '<div class="tree_progress">' +
            '<span class="chops_current" id="chops_current_' + player_id + '">0</span> / ' +
            '<span class="chops_required">' + treeDef.chops_required + "</span>" +
            "</div>" +
            '<div class="tree_points">Worth: ' + treeDef.points + " points</div>" +
    ...
    displayCutTree(player_id, cutTree) {
        ...
        var html =
            ...
            "background-position: " + bgPosXPct + "% " + bgPosYPct + "%; " +
            "background-size: " + bgSizeWidth + 'px auto;">' +
            '<div class="card_label">' + treeDef.name + " (" +
            (cutTree.points_value || treeDef.points) + " pts)</div></div>";
    ...
    createPlayAreaCard(cardDefId, cardId, label) {
        ...
        return (
            '<div class="play_area_card" id="play_card_' + cardId + '" style="' +
            "width: " + this.cardWidth + "px; " +
            "height: " + this.cardHeight + "px; " +
            "background-image: url(" + g_gamethemeurl + "img/red_cards.jpg); " +
            "background-position: " + bgPosXPct + "% " + bgPosYPct + "%; " +
            "background-size: " + bgSizeWidth + "px auto;" +
            '">' +
    ...
    displayEquipment(player_id, equipment) {
        ...
        if (!cardDefId) {
            var card = this.gamedatas.cards ? this.gamedatas.cards[equipment.card_id] : null;
    ...
    displayHelp(player_id, helpCard) {
        ...
        if (!cardDefId) {
            var card = this.gamedatas.cards ? this.gamedatas.cards[helpCard.card_id] : null;
            if (card) cardDefId = card.type_arg;
        }
    ...
    displayModifier(player_id, modifier) {
        ...
        if (!cardDefId) {
            var card = this.gamedatas.cards ? this.gamedatas.cards[modifier.card_id] : null;
            if (card) cardDefId = card.type_arg;
        }

        var cardDef = cardDefId ? this.gamedatas.jackCards[cardDefId] : null;
        var label = cardDef
            ? cardDef.name + " (" + (modifier.modifier_value > 0 ? "+" : "") + modifier.modifier_value + ")"
    ...
    onEnteringChoppingRollResult(args) {
        ...
        if (args.apprentice_die !== null && args.apprentice_die !== undefined) {
            ...
            html +=
                ...
                " &mdash; " + apprenticeText +
                "</div>";
        }

        html +=
            '<div class="roll_summary">' +
            (args.chops || 0) + " " + _("Chop(s)") + ", " +
            (args.breaks || 0) + " " + _("Break(s)") + ", " +
            (args.misses || 0) + " " + _("Miss(es)") +
            "</div>";
    ...
    onLeavingSelectTreesToSwitch() {
        document.querySelectorAll(".play_area_card.selectable").forEach(function (el) {
    ...
    onPlayerHandSelectionChanged() {
        ...
        } else if (items.length > 0 && this.bga.actions.checkAction("actPlayReaction", true)) {
            var card_id = items[0].id;
    ...
    notif_cardPlayed(args) {
        ...
        if (args.player_id == this.bga.players.getActivePlayerId() && this.playerHand) {
    ...
    notif_cardReturned(args) {
        ...
        if (args.player_id == this.bga.players.getCurrentPlayerId() && this.playerHand) {
            this.addCardToHand({
                id: args.card_id,
